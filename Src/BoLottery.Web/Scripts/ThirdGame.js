var thirdGame = {
    Init: function () {
        thirdGame.Bind();
        thirdGame.SetRowStyle("betHis");
        $("#currPeriod").text($("#lbPeriod").text());
        thirdGame.BetListRowFocus();
        thirdGame.ShowRun();
    },
    LoadLatestGame: function () {
        ...
        if (totalMls <= 0) {
            try {
                $.ajax({
                    type: "POST",
                    url: "/ScriptServices/UsersService.asmx/GetLatestByThirdGame",
                    ...
                    success: function (data) {
                        ...
                        $.map(jsonData, function (item) {
                            ...
                            $("span[code=myFirstAreaRate]").html(item.FirstAreaRate + "倍");
                            $("span[code=mySecondAreaRate]").html(item.SecondAreaRate + "倍");
                            $("span[code=myThirdAreaRate]").html(item.ThirdAreaRate + "倍");
                            $("span[code=myFourthAreaRate]").html(item.FourthAreaRate + "倍");
                        })

                        thirdGame.ShowRun();
                        thirdGame.OnBetHisClear();

                        setTimeout(thirdGame.BindRunHis, 25000);
                    }
                })
            }
            catch (e) {
    ...
    BindRunHis: function () {
        try {
            $.ajax({
                type: "POST",
                url: "/ScriptServices/UsersService.asmx/BindRunHisByThirdGame",
                contentType: "application/json; charset=utf-8",
                data: '',
                success: function (data) {
                    var jsonData = (new Function("return " + data.d))();
                    ...
                    var htmlAppend = "";
                    $.map(jsonData, function (item) {
                        htmlAppend += "<ul class=\"h_ul\"><li class=\"td mnone period\">" + item.Period + "</li>";
                        var ticketItems = $.trim(item.TicketNum).split(",");
                        var ticketItemsLen = ticketItems.length;
                        for (var i = 0; i < ticketItemsLen; i++) {
                            var currArr = ticketItems[i].split("-");
                            switch (currArr[0]) {
                                case "黑":
                                    normalAppend += "<a class=\"ci_bl\">" + currArr[1] + "</a>";
                                    break;
                                case "紅":
                                    normalAppend += "<a class=\"ci_rd\">" + currArr[1] + "</a>";
                                    break;
                                case "黃":
                                    normalAppend += "<a class=\"ci_yl\">" + currArr[1] + "</a>";
                                    break;
                                default:
                                    break;
                            }
                        ...
                        htmlAppend += "<li class=\"td total\">" + item.BetTotalPrice + "</li>";
                        htmlAppend += "<li class=\"td total\">" + item.PayTotalPrice + "</li>";
    ...
    ShowRun: function () {
        ...
        $("#lbRemainTime").text(thirdGame.CreateTimeByTotalMls(totalMls));

        if (totalMls > 0) {
            setTimeout(arguments.callee, 1000);
        }
        else {
            thirdGame.LoadLatestGame();
        }
    },
        ...
        itemsRows.each(function () {
            ...
            txts.each(function () {
                ...
                if (reg.test(currValue) && (parseInt(currValue) > 0)) {
                    ...
                    loopIndex++;
                    console.log("aaaa--" + currValue);
                    var currParent = currItem.parent().prev();
                    var aa = "";
                    if (currParent.attr("code") != undefined) {
                        aa += currParent.attr("code");
                    }
                    if ($.trim(currParent.text()).length > 0) {
                        if (aa.length > 0) aa += "-";
                        aa += $.trim(currParent.text());
                    }
                    if (loopIndex > 1) itemAppend += "|";
                    itemAppend += $_this.children(":first").text() + "," + aa + "," + currValue;
                }
            })
        })
    ...
    OnBetConfrm: function () {
        try {
            ...
            $.ajax({
                type: "POST",
                url: "/ScriptServices/UsersService.asmx/BetThirdGame",
                contentType: "application/json; charset=utf-8",
                ...
                success: function (data) {
                    ...
                    if (msg == 1) {
                        ...
                        thirdGame.OnAppendToBetHis();
                        thirdGame.OnBetClear();
                        jsHelper.GetUserPrice();
                    }
                    else {
